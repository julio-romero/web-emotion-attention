...
class DataProcessor:
    def __init__(self, web_data_path, imotion_data_path, output_dir):
        ...
        self.capture_handler = WebPageCapture(chrome_driver_path=os.getenv("CHROME_DRIVER_PATH"))
    ...
    def capture_mouse_activity(self, screen_number):
        mouse_activity_path = os.path.join(self.output_dir, f"screen_{screen_number}.txt")
    ...
    def parse_mouse_actions_to_dataframe(self, activity_file):
        ...
        with open(activity_file, encoding="utf-8") as file:
            for line in file:
                action_details = {}
                parts = line.split(" ")
                action_type = "moved" if "moved" in line else "clicked" if "clicked" in line else "scrolled"
                coords = [coord.split("-")[1].replace(",", "") for coord in parts if "-" in coord]
    ...
    def plot_data(self):
        for screenshot_path, activity_file in zip(self.screenshot_paths, self.mouse_activity_files):
...
# QA from here
    ...
    def process_web_data(self):
        ...
        self.web_data["Scroll Percentage"] = (
            self.web_data.groupby((self.web_data["URL"] != self.web_data["URL"].shift()).cumsum())["Scroll Percentage"]
    ...
    def merge_web_and_imotion_data(self):
        ...
        self.eye_tracking_data["Time From Start"] = (
            self.eye_tracking_data["Timestamp"] - self.eye_tracking_data["Timestamp"].iloc[0]
        )

        # Apply this difference to the 'web_data' timestamps
        self.eye_tracking_data["Aligned Timestamp"] = (
            self.web_data["Time (UTC)"].iloc[0] + self.eye_tracking_data["Time From Start"]
        )
